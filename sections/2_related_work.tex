...
Recent research has begun to address the complexities of narrative video. StoryDiffusion~\cite{storydiff} and DirecT2V~\cite{direct2v} attempt to maintain character consistency across multiple generations using reference attention or LLM-driven directors. However, as our experiments show, these models often suffer from \textbf{Prompt Bleeding}, where semantic information from Shot 1 contaminated Shot 2, preventing sharp narrative transitions.

\subsection{The Flaws of Legacy Evaluation}
Current benchmarks like VBench~\cite{vbench} rely on three types of metrics that we argue are ill-suited for T2MSV:

\textbf{1. Holistic Image Similarity:} StoryDiffusion~\cite{storydiff} uses average CLIP similarity between all frames to prove character consistency. This creates the \textit{Global Similarity Paradox}: if a model correctly changes the background as prompted, the holistic similarity \textit{decreases}. Thus, models are penalized for correct behavior. Our framework resolves this by using DINOv2-based subject masking to isolate identity from dynamics.
...
\textbf{3. VLM-as-a-Judge Limits:} While DirecT2V~\cite{direct2v} employs GPT-4V for narrative QA, language models are notoriously bad at catching pixel-level "morphing" artifacts—where one object slowly dissolves into another instead of cutting. Our mathematical \textbf{DSA} metric provides a more rigorous, objective measure of shot-level independent adherence.
