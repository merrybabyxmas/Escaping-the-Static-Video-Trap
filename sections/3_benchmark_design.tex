...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=\textwidth]{figures/images/fig2.jpg}
...
\begin{itemize}
    ...
    \item \textbf{Cut Sharpness ($\uparrow$):} These environmental jumps should be depicted as distinct cinematic cuts. High sharpness at the shot boundaries ensures that the scenes are rendered independently, preventing unnatural "morphing" artifacts.
    ...
    \item \textbf{Diagonal Semantic Alignment ($\uparrow$):} The model must explicitly follow the unique semantic instructions of each shot, guaranteeing that the environmental shift is driven by the prompt rather than random noise or context bleeding.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Diagonal Semantic Alignment ($\uparrow$):} Even with a static background, the model must accurately differentiate and execute fine-grained camera control instructions without confusing adjacent actions.
\end{itemize}
...
We utilize a masked DINOv2 similarity to measure identity preservation. For a video with $K$ shots, let $s_i$ be the representative frame of shot $i$, and $m_i$ be the corresponding subject mask. The score is defined as the mean pairwise cosine similarity between masked embeddings:
$$\mathcal{C}_{subj} = \frac{2}{K(K-1)} \sum_{i < j} \cos(\Phi_{DINO}(s_i \odot m_i), \Phi_{DINO}(s_j \odot m_j))$$
This decoupling ensures that intended environmental changes do not artificially degrade the identity score.

\subsubsection{Background Diversity ($\mathcal{D}_{bg}$)}
To identify the \textit{Static Trap}, we measure the variance of the background environment. Let $\overline{m}_i$ be the background mask ($1 - m_i$). $\mathcal{D}_{bg}$ is calculated as the mean distance from the average background embedding:
...
We quantify the distinctness of shot boundaries using LPIPS distance. Let $f_t$ be the frame at time $t$. For a triggered cut at $t_{cut}$, the sharpness is the perceptual distance peak:
...
Our core innovation, DSA, measures the model's ability to execute independent shot-level instructions without "prompt bleeding". We construct a $K \times K$ similarity matrix $M$ between shot frames and their respective prompts. To penalize static generation, we apply a column-wise softmax with logit scaling $\tau$ to obtain a probability matrix $P$:
...
$$DSA = \max \left( 0, \frac{\frac{1}{K}\sum_{i=1}^{K} P_{i,i} - \frac{1}{K}}{1 - \frac{1}{K}} \right)$$
