...
In Track B (Motion Control), where models must preserve the background, \textbf{LTX-Video}~\cite{HaCohen2024LTXVideo} shows promising results with a low diversity score (0.04). However, its low DSA score indicates that while the background is fixed, it fails to execute the camera motion instructions (e.g., panning vs zooming). Conversely, \textbf{StoryDiffusion} suffers from \textit{Background Hallucination} in this track (Diversity 0.51), proving that its narrative priors are entangled with environment rendering.
...
\midrule
CogVideoX~\cite{yang2024cogvideox} & 0.08 & 0.02 & 0.05 & 0.00 \\
SVD~\cite{blattmann2023stable} & 0.14 & 0.01 & 0.10 & 0.01 \\
LTX-Video~\cite{HaCohen2024LTXVideo} & 0.42 & 0.07 & 0.05 & 0.00 \\
StoryDiffusion~\cite{zhou2024storydiffusion} & 0.36 & 0.18 & 0.52 & 0.02 \\
FreeNoise~\cite{qiu2023freenoise} & 0.32 & 0.01 & 0.20 & 0.05 \\
AnimateDiff~\cite{guo2023animatediff} & 0.08 & 0.01 & 0.09 & 0.02 \\
\bottomrule
\end{tabular}
\end{table}
